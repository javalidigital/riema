...
jQuery(document).ready(function($) {
    ...
    $('.wpb_edit_form_elements .fa_vcit_icon_field').each(function() {
		...
		if ( $('[data-fa-vcit-css-admin]').length == 0 ) {
			 $('head').append('<link rel="stylesheet" href="' + cssUrl + 'css/font-awsome.css" type="text/css" data-fa-vcit-css-admin="1" />');
		}
		...
		iconKeyUpTimeout = setTimeout( function() {
			var $displayArea = $this.parent().find('.vcit_select_font_window');
			$displayArea.html('').show();

			$.each(vcab_icon_list, function(i) {
			    var rSearchTerm = new RegExp($this.val(),'i');
				if ( $this.val().indexOf('-') != -1 ) {
					rSearchTerm = new RegExp("^" + $this.val(),'i');
				}
			    if (vcab_icon_list[i].match(rSearchTerm)) {
					setTimeout( function() {
						$('<i class="fa ' + vcab_icon_list[i] + '"></i>').prependTo($displayArea);
					}, 10 );
			    }
			});
		}, 500 );
    ...
    $('.fa_vcit_icon_field').trigger('keyup');

	$('.wpb_edit_form_elements  .vcit_select_font_window').on('click', 'i', function() {
		var $field = $(this).parents('.vcit_select_font_window').parent().find('input');
		$field.val($(this).attr('class'))
		.prev().html('<i class="' + $field.val() + '"></i>');
	});

	$('.wpb_edit_form_elements .vcit_icon_filter').change(function() {
